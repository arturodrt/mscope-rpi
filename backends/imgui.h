...
#  ifdef IMGUI_USE_WCHAR32 // ImWchar [configurable type: override in imconfig.h
                           // with '#define IMGUI_USE_WCHAR32' to support
                           // Unicode planes 1-16]
...
SetWindowCollapsed(bool collapsed,
                   ImGuiCond cond
                   = 0); // (not recommended) set current window collapsed
...
IMGUI_API void SetWindowSize(const char *name, const ImVec2 &size,
                             ImGuiCond cond
                             = 0); // set named window size. set axis to 0.0f to
...
IMGUI_API bool DragInt(const char *label, int *v, float v_speed = 1.0f,
                       ...
                       = 0); // If v_min >= v_max we have no bound
...
IMGUI_API bool IsPopupOpen(const char *str_id,
                           ...
                           = 0); // return true if the popup is open.
...
BeginDragDropSource(ImGuiDragDropFlags flags
                    = 0); // call after submitting an item which may be dragged.
...
enum ImGuiWindowFlags_
{
  ...
  = 1 << 17, // No focusing toward this window with gamepad/keyboard navigation
  ...
  = 1 << 23, // [BETA] On child window: share focus scope, allow
             // gamepad/keyboard navigation to cross over parent border to this
...
#  ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ...
  = 1 << 30, // Obsoleted in 1.90: Use ImGuiChildFlags_AlwaysUseWindowPadding in
             // BeginChild() call.
#  endif
};
...
enum ImGuiChildFlags_
{
  ...
  = 1 << 2, // Allow resize from right border (layout direction). Enable .ini
            // saving (unless ImGuiWindowFlags_NoSavedSettings passed to window
            // flags)
...
enum ImGuiInputTextFlags_
{
  ...
  = 1 << 5, // Return 'true' when Enter is pressed (as opposed to every time the
  ...
  = 1 << 8, // Callback on each iteration. User code may query cursor position,
            // modify text buffer.
  ...
  = 1 << 18, // Callback on buffer capacity changes request (beyond 'buf_size'
             ...
             // string wants to be resized (for string types which hold a cache
             // of their Size). You will be provided a new BufSize in the
             // callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for
             // an example of using this)
  ImGuiInputTextFlags_CallbackEdit
  = 1 << 19, // Callback on any edit (note that InputText() already returns true
...
enum ImGuiTreeNodeFlags_
{
  ...
  = 1 << 11, // Extend hit box to the right-most edge, even if not framed. This
             // is not the default in order to allow adding other items on the
             // same line. In the future we may refactor the hit system to be
             // front-to-back, allowing natural overlaps and then this can
             // become the default.
...
enum ImGuiTabBarFlags_
{
  ...
  = 1 << 3, // Disable behavior of closing tabs (that are submitted with p_open
...
enum ImGuiTabItemFlags_
{
  ...
  = 1 << 2, // Disable behavior of closing tabs (that are submitted with p_open
            // != NULL) with middle mouse button. You may handle this behavior
            // manually on user's side with if (IsItemHovered() &&
            // IsMouseClicked(2)) *p_open = false.
  ...
  = 1 << 7, // Enforce the tab position to the right of the tab bar (before the
            // scrolling buttons)
  ImGuiTabItemFlags_NoAssumedClosure
  = 1 << 8, // Tab is selected when trying to close + closure is not immediately
            // assumed (will wait for user to stop submitting the tab).
            // Otherwise closure is assumed when pressing the X, so if you keep
            // submitting the tab may reappear at end of tab bar.
};
...
enum ImGuiHoveredFlags_
{
  ...
  = 1 << 0, // IsWindowHovered() only: Return true if any children of the window
            // is hovered
  ImGuiHoveredFlags_RootWindow
  = 1 << 1, // IsWindowHovered() only: Test from root window (top most parent of
  ...
  = 1 << 5, // Return true even if a popup window is normally blocking access to
            // this item/window
  ...
  = 1 << 11, // IsItemHovered() only: Disable using gamepad/keyboard navigation
  ...
  = 1 << 13, // Require mouse to be stationary for style.HoverStationaryDelay
             ...
             // item/window. Using the stationary test tends to reduces the need
             // for a long delay.
  ImGuiHoveredFlags_DelayNone
  = 1 << 14, // IsItemHovered() only: Return true immediately (default). As this
...
enum ImGuiDockNodeFlags_
{
  ...
  = 1 << 3, //       // Enable passthru dockspace: 1) DockSpace() will render a
            //       ImGuiCol_WindowBg background covering everything excepted
            //       the Central Node when empty. Meaning the host window should
            //       probably use SetNextWindowBgAlpha(0.0f) prior to Begin()
...
enum ImGuiDragDropFlags_
{
  ...
  = 1 << 0, // Disable preview tooltip. By default, a successful call to
            // BeginDragDropSource opens a tooltip so you can display a preview
            // or description of the source contents. This flag disables this
            // behavior.
  ...
  = 1
    << 3, // Allow items such as Text(), Image() that have no unique identifier
  ...
  = 1 << 10, // AcceptDragDropPayload() will returns true even before the mouse
             // button is released. You can then call IsDelivery() to test if
             // the payload needs to be delivered.
...
enum ImGuiConfigFlags_
{
  ...
  = 1 << 5, // Instruct backend to not alter mouse cursor shape and visibility.
            ...
            // you don't want to use SetMouseCursor() to change mouse cursor.
  ...
  = 1 << 14, // [BETA: Don't use] FIXME-DPI: Reposition and resize imgui windows
...
enum ImGuiBackendFlags_
{
  ...
  = 1 << 11, // Backend Platform supports calling io.AddMouseViewportEvent()
             // with the viewport under the mouse. IF POSSIBLE, ignore viewports
             // with the ImGuiViewportFlags_NoInputs flag (Win32 backend,
             // GLFW 3.30+ backend can do this, SDL backend cannot). If this
             // cannot be done, Dear ImGui needs to use a flawed heuristic to
...
enum ImGuiColorEditFlags_
{
  ...
  = 1 << 1, //              // ColorEdit, ColorPicker, ColorButton: ignore Alpha
            //              component (will only read 3 components from the
            //              input pointer).
  ImGuiColorEditFlags_NoPicker
  = 1 << 2, //              // ColorEdit: disable picker when clicking on color
            //              square.
  ...
  = 1
    << 19, //              // (WIP) ColorEdit: Currently only disable 0.0f..1.0f
  ...
  = 1 << 27, // [Input]      // ColorEdit, ColorPicker: input and output data in
             // RGB format.
  ImGuiColorEditFlags_InputHSV
  = 1 << 28, // [Input]      // ColorEdit, ColorPicker: input and output data in
             // HSV format.
...
enum ImGuiTableFlags_
{
  ...
  = 1
    << 3, // Enable sorting. Call TableGetSortSpecs() to obtain sort specs. Also
          // see ImGuiTableFlags_SortMulti and ImGuiTableFlags_SortTristate.
  ImGuiTableFlags_NoSavedSettings
  = 1 << 4, // Disable persisting columns order, width and sort settings in the
            // .ini file.
  ...
  = 1 << 12, // [ALPHA] Disable vertical borders in columns Body until hovered
             // for resize (borders will always appear in Headers). -> May move
             // to style Sizing Policy (read above for defaults)
  ...
  = 1 << 18, // Disable keeping column always minimally visible when ScrollX is
  ...
  = 1 << 20, // Disable clipping rectangle for every individual columns (reduce
  ...
  = 1 << 28, // Highlight column headers when hovered (may evolve into a fuller
             // highlight)
...
enum ImGuiTableColumnFlags_
{
  ...
  = 1 << 3, // Column will stretch. Preferable with horizontal scrolling
            // disabled (default if table sizing policy is _SizingStretchSame or
            // _SizingStretchProp).
  ...
  = 1 << 8, // Disable clipping for this column (all NoClip columns will render
  ...
  = 1 << 14, // Make the initial sort direction Ascending when first sorting on
             // this column (default).
  ImGuiTableColumnFlags_PreferSortDescending
  = 1 << 15, // Make the initial sort direction Descending when first sorting on
             // this column.
  ImGuiTableColumnFlags_IndentEnable
  = 1 << 16, // Use current Indent value when entering cell (default for column
             // 0).
...
struct ImGuiWindowClass
{
  ...
  ImGuiID FocusRouteParentWindowId; // ID of parent window for shortcut focus
                                    ...
                                    // from Parent Window will succeed when this
                                    // window is focused.
  ImGuiViewportFlags
...
enum ImDrawFlags_
{
  ...
  = 1 << 4, // AddRect(), AddRectFilled(), PathRect(): enable rounding top-left
            // corner only (when rounding > 0.0f, we default to all corners).
            // Was 0x01.
  ImDrawFlags_RoundCornersTopRight
  = 1 << 5, // AddRect(), AddRectFilled(), PathRect(): enable rounding top-right
            // corner only (when rounding > 0.0f, we default to all corners).
            // Was 0x02.
...
struct ImDrawList
{
  ...
  IMGUI_API void PathBezierCubicCurveTo(const ImVec2 &p2, const ImVec2 &p3,
                                        const ImVec2 &p4,
                                        int num_segments
                                        = 0); // Cubic Bezier (4 control points)
  IMGUI_API void
...
enum ImFontAtlasFlags_
{
  ...
  = 1 << 1, // Don't build software mouse cursors into the atlas (save a little
            // texture memory)
...
enum ImGuiViewportFlags_
{
  ...
  = 1 << 9, // Platform Window: Avoid merging this window into another host
            // window. This can only be set via ImGuiWindowClass viewport flags
            // override (because we need to now ahead if we are going to create
  ...
  = 1
    << 12, // Platform Window: Window is minimized, can skip render. When
...
static inline ImGuiKey GetKeyIndex(ImGuiKey key)
{
  IM_ASSERT(key >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END
            && "ImGuiKey and native_index was merged together and native_index "
               "is disabled by IMGUI_DISABLE_OBSOLETE_KEYIO. Please switch to "
               "ImGuiKey.");
  return key;
}
#  endif
