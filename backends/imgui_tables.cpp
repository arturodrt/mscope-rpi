                         ...
                         float inner_width)
{
  ...
  if (use_child_window)
  {
    ...
    if ((table_last_flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY))
        == 0)
...
#  endif
      ...
      && table->InstanceCurrent == table->InstanceInteracted)
  {
    ...
    const float new_width = ImTrunc(new_x2 - column->MinX - table->CellSpacingX1
                                    - table->CellPaddingX * 2.0f);
    ...
    if (draw_cell_bg_color)
    {
           ...
           cell_data <= cell_data_end; cell_data++)
      {
        ...
        cell_bg_rect.Min.x = ImMax(
            cell_bg_rect.Min.x,
            column->ClipRect.Min.x); // So that first column after frozen one
...
float ImGui::TableGetHeaderRowHeight()
{
  ...
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
      if ((table->Columns[column_n].Flags & ImGuiTableColumnFlags_NoHeaderLabel)
          == 0)
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
      ...
      if (pass == 0)
      {
        ...
        while (label_name < label_name_end)
        {
          ...
          pivot_out.x += flip_label ? line_off_curr_x - line_off_step_x
                                    : line_off_curr_x;
