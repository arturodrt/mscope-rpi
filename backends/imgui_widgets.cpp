    ...
    if (!nav_activated_by_inputs && (item_flags & ImGuiItemFlags_ButtonRepeat))
    {
      ...
      nav_activated_by_inputs
          = CalcTypematicRepeatAmount(t1 - g.IO.DeltaTime, t1,
                                      g.IO.KeyRepeatDelay, g.IO.KeyRepeatRate)
            > 0;
    }
...
{
  ImGuiGetNameFromIndexOldToNewCallbackData old_to_new_data
      = {user_data, old_getter};
                       ...
                       const char *format, ImGuiSliderFlags flags)
{
  ...
  // Drag behavior
  const bool value_changed = DragBehavior(id, data_type, p_data, v_speed, p_min,
                                          p_max, format, flags);
  if (value_changed)
    MarkItemEdited(id);
          ...
          if (buf_dirty)
          {
            ...
            if (callback_data.BufTextLen > backup_current_text_length
              ...
              state->TextW.resize(
                  state->TextW.Size
                  + (callback_data.BufTextLen
                     - backup_current_text_length)); // Worse case scenario
                                                     // resize
  ...
  if (render_cursor || render_selection)
  {
    ...
    if (render_cursor && state->CursorFollow)
    {
      ...
      if (is_multiline)
      {
        ...
        else if (cursor_offset.y - (inner_size.y - style.FramePadding.y * 2.0f)
                 >= scroll_y)
          scroll_y
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
  ...
  {
    is_open
        = storage->GetInt(id, (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0)
          != 0;
  }
                             ...
                             ImGuiTreeNodeFlags flags)
{
  ...
  if (p_visible != NULL)
  {
    ...
    float button_x = ImMax(g.LastItemData.Rect.Min.x,
                           g.LastItemData.Rect.Max.x - g.Style.FramePadding.x
                               - button_size);
    ...
    int nav_item_idx)
{
  ...
  for (int idx = 0; idx < items_count; idx++)
  {
    ...
    if (ImStrimatchlen(req->SearchBuffer,
                       req->SearchBuffer + req->SingleCharSize, item_name)
        < req->SingleCharSize)
      continue;
...
void ImGui::EndMenuBar()
{
  ...
  window->DC.MenuBarOffset.x
      = window->DC.CursorPos.x
        - window->Pos
              .x; // Save horizontal position so next append can reuse it. This
        ...
        && menuset_is_open) // Click an open menu again to close it
    {
      ...
      want_open = menu_is_open = false;
             ...
             || (hovered && menuset_is_open
                 && !menu_is_open)) // First click to open, then hover to open
                                    // others
    {
      want_open = true;
    }
  ...
  if (select_dir != 0)
        ...
        = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId))
    {
      ...
      while (tab_to_scroll_to == NULL)
      {
        ...
        // visible
        tab_to_scroll_to = &tab_bar->Tabs[(target_order >= 0
