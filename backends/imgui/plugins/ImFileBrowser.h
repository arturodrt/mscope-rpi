...
private:
  ...
  // IMPROVE: overflow when selectedFilename_.length() > inputNameBuf_.size() -
  // 1
...
inline void
ImGui::FileBrowser::SetFileFilters(const std::vector<const char *> &fileFilters)
{
  typeFilters_ = fileFilters;
  typeFilterIndex_ = 0;
}

inline void ImGui::FileBrowser::SetLabels(const char *folderLabel,
...
{
  fileRecords_ = {FileRecord{
      true, "..", std::string(directoryLabel_) + std::string(" ..")}};
  ...
  for (auto &p : std::filesystem::directory_iterator(pwd))
  {
    ...
    rcd.showName = (rcd.isDir ? directoryLabel_ : fileLabel_) + std::string(" ")
