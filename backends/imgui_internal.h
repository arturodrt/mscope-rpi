...
#    pragma warning(push)
#    pragma warning(                                                           \
        disable : 4251) // class 'xxx' needs to have dll-interface to be used by
                        // clients of struct 'xxx' // when IMGUI_API is set
...
IMGUI_API ImU64 ImFileWrite(const void *data, ImU64 size, ImU64 count,
                            ImFileHandle file);
#  else
#    define IMGUI_DISABLE_TTY_FUNCTIONS // Can't use stdout, fflush if we are
                                        // not using default file functions
#  endif
...
enum ImGuiItemFlags_
{
  ...
  = 1 << 5, // false     // Disable MenuItem/Selectable() automatically closing
            // their popup window
  ...
  = 1
    << 6, // false     // [BETA] Represent a mixed/indeterminate value,
          ...
          // supported by Checkbox() (later should support all sorts of widgets)
...
enum ImGuiItemStatusFlags_
{
  ...
  = 1 << 8, // [WIP] Set when item is overlapping the current clipping rectangle
            // (Used internally. Please don't use yet: API/system will change as
            // we refactor Itemadd()).
...
enum ImGuiButtonFlagsPrivate_
{
  ...
  = 1 << 5, // [Default] return true on click + release on same item <-- this is
            // what the majority of Button are using
  ...
  = 1 << 9, // return true when held into while we are drag and dropping another
  ...
  = 1 << 15, // vertically align button to match text baseline - ButtonEx() only
             // // FIXME: Should be removed and handled by SmallButton(), not
             // possible currently because of DC.CursorPosPrevLine
...
enum ImGuiSelectableFlagsPrivate_
{
  ...
  = 1 << 21, // (WIP) Auto-select when moved into. This is not exposed in public
             // API as to handle multi-select and modifiers we will need user to
...
enum ImGuiTreeNodeFlagsPrivate_
{
  ...
  = 1 << 21, // (FIXME-WIP) Turn Down arrow into an Up arrow, but reversed trees
             // (#6517)
};
...
enum ImGuiInputFlags_
{
  ...
  = 1 << 6, // Stop repeating when released OR if keyboard mods are leaving the
  ...
  = 1
    << 11, // Further accesses to key data will require EXPLICIT owner ID
           // (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared when
           // the key is released or at end of each frame if key is released.
  ...
  = 1 << 12, // (Default) Honor focus route: Accept inputs if window is in focus
             // stack. Deep-most focused window takes inputs. ActiveId takes
  ...
  = 1 << 14, // Register route globally (medium priority: unless an active item
             // registered the route, e.g. CTRL+A registered by InputText will
  ...
  = 1 << 17, // Global routes will not be applied if underlying background/void
             // is focused (== no Dear ImGui windows are focused). Useful for
...
enum ImGuiScrollFlags_
{
  ...
  = 1 << 1, // If item is not visible: scroll as little as possible on Y axis to
            // bring item back into view [default for Y axis for windows that
            // are already visible]
  ...
  = 1
    << 6, // Disable forwarding scrolling to parent window if required to keep
...
enum ImGuiNavMoveFlags_
{
  ...
  = ImGuiNavMoveFlags_LoopX | ImGuiNavMoveFlags_LoopY | ImGuiNavMoveFlags_WrapX
    | ImGuiNavMoveFlags_WrapY,
  ...
  = 1 << 4, // Allow scoring and considering the current NavId as a move target
            ...
            // are pressing PageDown from the bottom-most item we need to stay
            // in place)
  ...
  = 1
    << 14, // Do not alter the visible state of keyboard vs mouse nav highlight
};
...
enum ImGuiTypingSelectFlags_
{
  ...
  = 1 << 1, // Allow "single char" search mode which is activated when pressing
...
enum ImGuiOldColumnFlags_
{
  ...
  = 1 << 4, // Restore pre-1.51 behavior of extending the parent window contents
            // size but _without affecting the columns width at all_. Will
            // eventually remove.
...
enum ImGuiDockNodeFlagsPrivate_
{
  ...
  = 1 << 10, // Saved // A dockspace is a node that occupy space within an
             // existing user window. Otherwise the node is floating and create
             // its own window.
  ...
  = 1
    << 18, //       // Any docked window will be automatically be focus-route
           //       chained (window->ParentWindowForFocusRoute set to this) so
           //       Shortcut() in this window can run when any docked window
           //       is focused.
           // Disable docking/undocking actions in this dockspace or
           // individual node (existing docked nodes will be preserved) Those
           // are not exposed in public because the desirable
  ...
  = 1 << 20, //       // Disable other windows/nodes from being docked over this
             //       node.
  ImGuiDockNodeFlags_NoDockingOverOther
  = 1 << 21, //       // Disable this node from being docked over another window
             //       or non-empty node.
  ImGuiDockNodeFlags_NoDockingOverEmpty
  = 1 << 22, //       // Disable this node from being docked over an empty node
...
enum ImGuiTabItemFlagsPrivate_
{
  ...
  = 1 << 20, // Track whether p_open was set or not (we'll need this info on the
             // next frame to recompute ContentWidth during layout)
...
IMGUI_API bool Shortcut(ImGuiKeyChord key_chord, ImGuiID owner_id = 0,
                        ImGuiInputFlags flags = 0);
IMGUI_API bool
SetShortcutRouting(ImGuiKeyChord key_chord, ImGuiID owner_id,
                   ImGuiInputFlags flags
                   = 0); // owner_id needs to be explicit and cannot be 0
